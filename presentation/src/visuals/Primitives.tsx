...
export const formatNumber = (n:number, decimals=2) => n.toLocaleString('es-ES',{minimumFractionDigits:decimals,maximumFractionDigits:decimals});
...
export function NumberValue({value,decimals=2,suffix=''}:{value:number;decimals?:number;suffix?:string}) {
  ...
  return <span data-value={value} className="tabular">{formatNumber(value,decimals)}{suffix}</span>;
}
export function RangeControl({label,value,onChange,min=0,max=.1,step=.005,sub}: {label:string;value:number;onChange:(n:number)=>void;min?:number;max?:number;step?:number;sub?:string}) {
  return <label className="range-control"><span>{label}<output>{formatNumber(value*100,1)}%</output></span><input type="range" aria-label={label} min={min} max={max} step={step} value={value} onChange={e=>onChange(Number(e.currentTarget.value))}/>{sub&&<small>{sub}</small>}</label>;
}
