...
export function BondGenerations({step}:{step:number}) {
  return <div className="generations"><div className="market-context"><span className="spain-flag"/>España · Bonos y Obligaciones del Estado <small>Ejemplo ilustrativo</small></div>
    ...
    <Build at={3} step={step} className="generation-conclusion"><CalendarClock/><p><strong>Una misma zona: ~5Y.</strong><span>Tres referencias distintas. Tres cupones que permanecen fijos.</span></p><div className="residual-cluster">{generations.map(b=><span key={b.id}><b>{b.id}</b>{euro(b.residual,1)}Y</span>)}</div></Build>
  </div>;
}
...
export function MarketCurve({step,chosen,onChoose}:{step:number;chosen:string|null;onChoose:(id:string)=>void}) {
  ...
  return <div className="market-curve"><div className="market-context"><span className="spain-flag"/>España · Curva de YTM <small>30 observaciones ilustrativas · no cotizaciones</small></div>
    <div className="curve-layout"><div className="curve-plot"><svg viewBox="0 0 780 350" role="img" aria-label="Curva ilustrativa de YTM. Vida residual de cero a cincuenta años en escala lineal, rendimiento del uno al cinco por ciento.">
    ...
    <aside className="curve-reading"><span className="panel-overline">LEER UN PUNTO</span><h2>{selected.id}</h2><div><span>Vida residual</span><strong>{euro(selected.maturityYears,1)}<small> años</small></strong></div><div><span>YTM</span><strong>{percent(selected.ytm)}</strong></div><p>Selecciona otro punto para leer sus coordenadas.</p><Build at={3} step={step}><div className="benchmark-list"><b>Referencias benchmark</b><span>2Y · 3Y · 5Y · 7Y<br/>10Y · 15Y · 30Y · 50Y</span><small>Puntos claros: ocho referencias de la nube.</small></div></Build></aside></div>
  </div>;
}
...
export function RecapFoundations({step}:{step:number}) {
  return <div className="recap-bands"><section className="recap-band"><div className="recap-topic"><span>01</span><h2>El contrato</h2><p>Qué se promete.</p></div><div className="recap-contract"><div className="recap-exchange"><Landmark/><span>Emisor</span><div><b>← Capital hoy</b><b>Pagos futuros →</b></div><FileText/><span>Inversor</span></div><div className="recap-terms"><span>c <b>4%</b></span><span>N <b>100 €</b></span><span>Principal <b>100 €</b></span><span>Plazo <b>5 años</b></span><span>Frecuencia <b>Anual</b></span></div><Build at={1} step={step}><MiniFlows coupon={4} showToday/><p className="recap-small">Cupón = 4% × 100 € = 4 € · Último pago = 4 + 100 = 104 €</p></Build></div></section>
    <Build at={2} step={step} className="recap-band"><div className="recap-topic"><span>02</span><h2>El valor</h2><p>Qué representa hoy.</p></div><div className="recap-valuation"><Formula>{'P_0 = \\sum_{t=1}^{n}\\frac{CF_t}{(1+r_t)^t}'}</Formula><div className="recap-price-equation"><Formula>{'\\frac{4}{1+r_1}+\\frac{4}{(1+r_2)^2}+\\frac{4}{(1+r_3)^3}+\\frac{4}{(1+r_4)^4}+\\frac{104}{(1+r_5)^5}'}</Formula></div><p><strong>100 €</strong> cuando todas las tasas son 4%.</p></div></Build></div>;
}
