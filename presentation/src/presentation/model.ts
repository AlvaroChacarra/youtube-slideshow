...
export const euro = (n: number, decimals = 2) =>
  n.toLocaleString("es-ES", {
    minimumFractionDigits: decimals,
    maximumFractionDigits: decimals,
  });
export const texNumber = (n: number, decimals = 2) =>
  euro(n, decimals).replace(",", "{,}");
...
export function couponCase(coupon: number, yieldRate = 0.04, reinvestment = 0) {
  ...
  const wealth = terminalWealth({
    flows,
    horizonYears: 5,
    annualEffectiveReinvestmentRate: reinvestment,
  });
  return {
    flows,
    price,
...
export function discountedFlows(lastRate: number) {
  return canonicalFlows.map((f) => ({
    ...f,
    rate: f.period === 5 ? lastRate : 0.04,
    pv: presentValue({
      amount: f.amount,
      annualEffectiveRate: f.period === 5 ? lastRate : 0.04,
      timeYears: f.timeYears,
    }),
  }));
}
